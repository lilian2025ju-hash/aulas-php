...
<body>
<h1>Trabalando com funções</h1>
<hr>

<h2>Função como procedimento( ou sub-rotina)</h2>
<p>procedimentos não tetornam nada.</p>
...
$precoProduto = somar ($precoProdutoA, $precoProdutoB);
 
 ?>
 <p>Resultado 4:<?= $precoProduto ?></p>
//Utilizando função como parte de condição de um if
 
<?php if(somar (100, 500) > 1200): ?>
    <p class = "text-sucess">meta atingida!</p>
    <?php else: ?>
        <p class ="test-danger">Não foi dessa vez</p>
        <?php endif; ?>

        <hr>

<h2>Função com parãmetros opcionais</h2>
<?php 
/*Neste caso, deixamos o parâmetro pessoa com uma valor padrão(no exemplo, uma string vazia)*/
